...
st.caption("Tell me what you want - I will search, rate, and order the best match for you")
...
with st.sidebar:
    ...
    st.caption("Upload a photo/image of a product, so i will find similar products in our store.")
    ...
    if uploaded_file and st.button("Find similar products", use_container_width=True): 
        ...
        prompt = f"I uploaded a product image. please analyze it and find similar products in the store. Image path is {image_path}"
...
#------------------------------------------------------------------------------------
# Run agent if there is an unprocessed message (image upload trigger this)
#------------------------------------------------------------------------------------
if(
    st.session_state.messages
...
if prompt := st.chat_input("eg. I want organic honey under $15 with 4+ rating"):
    st.session_state.messages.append({"role": "user", "content": prompt})
